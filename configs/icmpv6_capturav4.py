...
def save_bindings():
    valid = []
    now = datetime.utcnow().timestamp()  # Tiempo actual en segundos

    for b in bindings.values():
        if not (b["ipv6_link_local"] or b["ipv6_globals"]):
            continue  # Saltar si no tiene direcciones

        # Calcular cuándo expira esta entrada
        # Usamos el tiempo actual + duración configurada
        expires_at = now + TEMP_VALID_LFT

        cleaned = {
            "mac": b["mac"],
            "interface": b["interface"],
            "ipv6_link_local": b["ipv6_link_local"],
            "ipv6_globals": list(set(b["ipv6_globals"])),  # sin duplicados
            "timestamp": b["timestamp"],
            "expires_at": expires_at  # momento de expiración
        }
    ...
    print(f"[SAVE] {len(valid)} bindings guardados, expiran en ~{TEMP_VALID_LFT}s")

# === Clasificar si una dirección IPv6 es global (no link-local ni especial) ===
def is_global_ipv6(ip):
    return not ip.startswith("fe80::") and not ip.startswith("ff") and ":" in ip

# === Procesar paquetes ICMPv6 (RS, NS, NA) ===
