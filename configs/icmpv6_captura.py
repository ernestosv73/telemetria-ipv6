...
# Función para convertir un IPv6 tipo EUI-64 a MAC
def ipv6_to_mac(ipv6):
    ...
    eui64 = parts[-2:]
    if len(eui64[0]) == 1:
        eui64 = "".join([p.zfill(4) for p in eui64])
    else:
        eui64 = "".join(eui64)
    ...
    try:
        first_byte = format(int(eui64[:2], 16) ^ 0x02, '02x')  # Invertir bit U/L
...
def flush_block(interface_data):
    for block_info in interface_data:
        ...
        for line in current_block:
            ...
            if "neighbor solicitation" in line or "router solicitation" in line or "router advertisement" in line:
                is_valid_packet = True

            # Extraer IPv6 origen
            match_ipv6_src = re.search(r'IP6 ([0-9a-fA-F:]+) >', line)
            if match_ipv6_src:
        ...
        # Si no hay MAC explícita, intentar desde IPv6 origen
